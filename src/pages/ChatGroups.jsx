...
const ChatGroups = () => {
    ...
    useEffect(() => {
        if (selectedGroupId) {
            sessionStorage.setItem('ADMIN_ACTIVE_GROUP', selectedGroupId);
        } else {
            sessionStorage.removeItem('ADMIN_ACTIVE_GROUP');
        }
    }, [selectedGroupId]);
    ...
    // Clear on unmount
    useEffect(() => {
        return () => sessionStorage.removeItem('ADMIN_ACTIVE_GROUP');
    }, []);
    ...
    useEffect(() => {
        if (location.state?.groupId) {
            const targetGroup = groups.find(g => g.id === location.state.groupId);
            if (targetGroup) {
    ...
    const handleCreateGroup = async (e) => {
        ...
        try {
            ...
            if (response?.data?.id) {
                // Ideally set selectedGroupId here
            }
        } catch (error) {
