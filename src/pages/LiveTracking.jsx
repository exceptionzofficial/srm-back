...
const LiveTracking = () => {
    ...
    const loadData = async () => {
        try {
            ...
            if (data.success) {
                // ADMIN PORTAL REQUIREMENT: Show ONLY active staff (Online, Tracking, or Traveling)
                // This removes unwanted offline entries
                const activeEmployees = data.employees.filter(emp => emp.isOnline || emp.isTracking || !!emp.tripDetails);
                ...
                updateMapMarkers(activeEmployees);
            }
        } catch (error) {
                                            ...
                                            if (googleMapRef.current && emp.lastLocation) {
                                                ...
                                                if (!isNaN(lat) && !isNaN(lng)) {
                                                    ...
                                                    if (infoWindowsRef.current[emp.employeeId]) {
                                                        ...
                                                        infoWindowsRef.current[emp.employeeId].open(googleMapRef.current, markersRef.current[emp.employeeId]);
                                                    }
                                                }
                                            }
                                        }}
